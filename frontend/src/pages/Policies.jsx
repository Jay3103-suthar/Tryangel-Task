...
export default function Policies() {
  ...
  const fetchData = async () => {
    try {
      const [{ data: pol }, { data: cl }, { data: cat }] = await Promise.all([
        API.get('/policies'),
        API.get('/clients'),
        API.get('/categories')
      ]);
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (editing) {
        await API.put(`/policies/${editing}`, fd, {
  ...
  const startEdit = (p) => {
    ...
    setForm({
      ...
      issueDate: new Date(p.issueDate).toISOString().slice(0, 10),
              ...
              <tbody>
                {policies.map((p) => (
                  <tr key={p._id}>
                    ...
                    <td>
                      {p.attachmentUrl ? (
                        <a
                          ...
                          href={`${BACKEND_URL}${p.attachmentUrl}`}
                          target="_blank"
                          rel="noreferrer"
                        >
                          View
                    ...
                    <td>
                      <button className="btn btn-ghost mr-2" onClick={() => startEdit(p)}>Edit</button>
